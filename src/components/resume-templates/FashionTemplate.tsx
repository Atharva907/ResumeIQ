export default function FashionTemplate({ data }: { data: any }) {
  return (
    <div className="p-8 bg-gradient-to-br from-pink-50 to-purple-50 text-gray-800" id="resume-preview">
      {/* Fashion Header */}
      <header className="mb-8 pb-6 border-b-2 border-purple-300">
        <div className="text-center">
          ...
          <h2 className="text-xl text-purple-700 mb-4">{data.title || "Fashion Professional"}</h2>
          <div className="flex justify-center gap-6 text-sm text-purple-600">
            {data.contact?.email && <span>{data.contact.email}</span>}
            {data.contact?.phone && <span>{data.contact.phone}</span>}
            {data.contact?.location && <span>{data.contact.location}</span>}
            {data.contact?.instagram && <span>{data.contact.instagram}</span>}
          </div>
        </div>
      </header>
