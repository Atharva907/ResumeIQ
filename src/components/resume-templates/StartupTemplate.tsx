export default function StartupTemplate({ data }) {
  return (
    <div className="p-8 bg-gradient-to-br from-indigo-50 to-purple-50 text-gray-800" id="resume-preview">
      {/* Startup Header */}
      <header className="mb-8 pb-6 border-b-2 border-indigo-600">
        <div className="flex justify-between items-start">
          <div>
            ...
            <h2 className="text-xl text-indigo-700 mb-4">{data.title || "Startup Professional"}</h2>
          ...
          <div className="text-right">
            <div className="text-3xl font-bold text-indigo-600">
              {data.startupMetrics?.companies || "3"}
            </div>
            <div className="text-sm text-indigo-600">Companies Founded</div>
            <div className="text-2xl font-bold text-indigo-600 mt-2">
              ${data.startupMetrics?.funding || "10M+"}
            </div>
            <div className="text-sm text-indigo-600">Funding Raised</div>
          </div>
        </div>
      </header>
              ...
              <div className="p-4 bg-white rounded-lg shadow-md">
                <div className="flex flex-wrap gap-2">
                  {data.skills.map((skill, i) => (
                    <span key={i} className="px-3 py-1 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-full text-sm text-indigo-700">{skill}</span>
                  ))}
                </div>
              </div>
            </section>
            ...
            <section className="mb-8">
              ...
              {data.startups.map((startup, i) => (
                <div key={i} className="mb-4 p-4 bg-white rounded-lg shadow-md">
                  ...
                  {startup.description && <p className="text-gray-700 text-sm mb-2">{startup.description}</p>}
                  {startup.funding && (
